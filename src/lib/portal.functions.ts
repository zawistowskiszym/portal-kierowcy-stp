...
// Driver requests their assigned vehicle to be spawned in the Roblox game.
// Queues a `spawn_vehicle` command that the game picks up via GET /api/public/roblox/driver.
export const requestVehicleSpawn = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .handler(async ({ context }) => {
    const { supabase, userId } = context;
    const today = new Date().toISOString().slice(0, 10);
    ...
    const { data: duty, error: dErr } = await supabase
      .from("duties")
      .select(
        "id, duty_number, route, depot, vehicle_id, vehicle_label, vehicles:vehicle_id(id, vehicle_number, model, fuel, depot)",
      )
      .eq("assigned_to", userId)
      .eq("duty_date", today)
      .maybeSingle();
    if (dErr) throw new Error(dErr.message);
    if (!duty) throw new Error("Brak przydzielonej służby na dziś.");
    const v: any = (duty as any).vehicles ?? null;
    if (!duty.vehicle_id && !duty.vehicle_label) {
      throw new Error("Do tej służby nie przypisano pojazdu. Skontaktuj się z dyspozytorem.");
    }
    ...
    if (!profile?.roblox_username) {
      throw new Error("Brak nazwy użytkownika Roblox w profilu. Uzupełnij ją w zakładce Profil.");
    }

    // Reuse a still-pending spawn command instead of stacking duplicates.
    const { data: pending } = await supabase
      .from("roblox_commands")
      .select("id")
      .eq("target_user_id", userId)
      .eq("type", "spawn_vehicle")
      .is("acked_at", null)
      .order("created_at", { ascending: false })
      .limit(1);
    if (pending && pending.length > 0) {
      return { ok: true, command_id: pending[0].id, reused: true };
    }
    ...
    const payload = {
      ...
      vehicle_label: duty.vehicle_label ?? v?.vehicle_number ?? null,
    ...
    return { ok: true, command_id: inserted.id, reused: false };
  });
